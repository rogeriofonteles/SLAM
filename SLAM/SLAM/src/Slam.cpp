...
void Slam::ekfSlam(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& odom, const sensor_msgs::LaserScan::ConstPtr& scan){
    ...
    for(std::vector<InterestPoint*>::iterator it2=featuresCurrent.begin(); it2 != featuresCurrent.end(); it2++){   
        ...
        if(counter_real == features.size()){
        	...
        	result.first[result.first.size()-1] = z_diff_real[2];
        	...
        	for(int i=0; i<3; i++)
        	    for(int j=0; j<3; j++)
        	        P_rr(i,j) = result.second(i,j);
        	      
        	range r(result.second.size1()-3, result.second.size1());      
        	range r2(0, 3);
        	range r3(0, result.second.size1()-3);
        	range r4(3, result.second.size1());
        	
        	
        	matrix<double> temp_mult = prod(J_r, project(result.second, r2, r2));
        	matrix<double> temp_mult2 = prod(J_l, Q);
        	project(result.second, r, r) = prod(temp_mult, trans(J_r)) + prod(temp_mult2, trans(J_l));
        	
        	project(result.second, r, r4) = prod(J_r, project(result.second, r2, r3));
        	project(result.second, r4, r) = trans(prod(J_r, project(result.second, r, r)));
